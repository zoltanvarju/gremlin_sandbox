...
# TAKEN FROM: https://github.com/awslabs/amazon-neptune-tools/blob/master/csv-gremlin/csv-gremlin.py
...
class NeptuneCSVReader:
    ...
    def get_batch_sizes(self):
        return {"vbatch": self.vertex_batch_size, "ebatch": self.edge_batch_size}
    ...
    def process_edges(self, reader):
        ...
        if not "~from" in reader.fieldnames or not "~to" in reader.fieldnames:
            self.print_error(
                "For edges, the header row must include both ~from and ~to columns"
            )
            errors = True
        ...
        if len(kt) > 1:
            if kt[1].endswith("[]"):
                if self.mode == self.EDGE:
                    self.print_error(
                        "Only vertices can have Set cardinality properties"
                    )
                    sys.exit(1)
                else:
                    kt[1] = kt[1][0:-2]
    ...
    def process_edge_row(self, r):
        ...
        if seen != 0xF:
            self.print_error(
                "For edge data, values must be provided for ~id,~label,~from and ~to"
            )
            edge = ""
        else:
    ...
    def process_csv_file(self, fname):
        ...
        try:
            ...
            reader = csv.DictReader(fname.split("\n"), escapechar="\\")
